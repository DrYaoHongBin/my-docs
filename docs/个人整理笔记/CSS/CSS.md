...
type：定义所链接文档的类型，在这里需要指定为“text/CSS”，表示链接的外部文件为CSS样式表（现在一般都不用了）。
...
~~~
1. 样式显示效果跟HTML元素中的类名先后顺序没有关系,受CSS样式书写的上下顺序有关（比如第4条，bottom和J_Bottom哪个放前哪个放后没有区别）。
2. 各个类名中间用空格隔开。
~~~
...
交集选择器由两个选择器构成，其中第一个为标签选择器，第二个为class选择器，两个选择器之间不能有空格，如h3.special。（其实也可以写成h3#id，不过id已经是唯一的了，这样写没有意义）
...
（p标签比较特殊，不能放其他块级元素，因此如果一篇文章里面既有图片也有文字就不要用p标签了，使用div标签包裹，h1这些文本块标签也是。

<p><div></div></p>会变成<p></p><div></div><p></p>）
...
行内元素和行内块元素，多模块之间还有间隔。

![1532512011966](C:\Users\16428\Desktop\笔记\CSS笔记\media\1532512011966.png)
...
（4）行内元素只能容纳文本或则其他行内元素。（a特殊，a标签里面还可以放块级元素）
...
<img src="D:/%E6%88%91%E7%9A%84%E5%89%8D%E7%AB%AF/html%E5%92%8Ccss/%E7%AC%AC%E5%9B%9B%E5%A4%A9%E8%A7%86%E9%A2%91/%E7%AC%AC%E5%9B%9B%E5%A4%A9%E8%A7%86%E9%A2%91/%E5%89%8D%E7%AB%AF%E5%9F%BA%E7%A1%80%E7%AC%AC%E5%9B%9B%E5%A4%A9/%E7%AC%94%E8%AE%B0/media/line1.png"  />

<img src="D:/%E6%88%91%E7%9A%84%E5%89%8D%E7%AB%AF/html%E5%92%8Ccss/%E7%AC%AC%E5%9B%9B%E5%A4%A9%E8%A7%86%E9%A2%91/%E7%AC%AC%E5%9B%9B%E5%A4%A9%E8%A7%86%E9%A2%91/%E5%89%8D%E7%AB%AF%E5%9F%BA%E7%A1%80%E7%AC%AC%E5%9B%9B%E5%A4%A9/%E7%AC%94%E8%AE%B0/media/line2.png"  />
...
<img src="D:/%E6%88%91%E7%9A%84%E5%89%8D%E7%AB%AF/html%E5%92%8Ccss/%E7%AC%AC%E5%9B%9B%E5%A4%A9%E8%A7%86%E9%A2%91/%E7%AC%AC%E5%9B%9B%E5%A4%A9%E8%A7%86%E9%A2%91/%E5%89%8D%E7%AB%AF%E5%9F%BA%E7%A1%80%E7%AC%AC%E5%9B%9B%E5%A4%A9/%E7%AC%94%E8%AE%B0/media/1.png"  />
...
(不是所有的样式都会被继承，比如<div><p></p></div>,设置div的height为100,p的height不会变成100)
...
![1532572199107](C:\Users\16428\Desktop\笔记\CSS笔记\media\1532572199107.png)
...
![1532572113665](C:\Users\16428\Desktop\笔记\CSS笔记\media\1532572113665.png)
...
1. 使用了 !important声明的规则（如果是在继承中，父类color使用了！important，如果子类自己定义了color,则使用的是子类自己的color,因为继承的影响父类的权重变成了0000）。
...
3. position后面可以跟访问名称，他们之间可以没有上下顺序，比如background-position: bottom right 和background-position: right bottom是一样的
4. 如果x和y只写了其中一个（方位名词或者是精确单位都一样），则另外一个默认是居中的，center
...
以前学过的html表格边框很粗，这里只需要CSS一句话就可以美观起来。 让我们真的相信，CSS就是我们的白马王子（白雪公主），如果不用下面的属性，table取消单元格与单元格之间的间隙后两个相邻边框会合在一起而不是重叠，因此看起来会比较粗。
...
重点：如果块元素设置了width和heigth（不添加则没有影响）,则添加padding或者是border-width后盒子的大小也会跟着改变，因此如果想要盒子大小不变，需要根据padding和border-width的值对width和heigth做出相应的修改。
...
​	**子类的margin和padding都不会撑开父盒子，如果超出父盒子的范围，则会超出去，margin和padding的值可以是负数
...
~~~css
.header{ width:960px; margin:0 auto;}，上下使用auto是没有效果的，auto的作用是填满
~~~

### 文字盒子居中图片和背景区别

1.  文字水平居中是  text-align: center（文字、行内元素和行内块元素才能用这种方式对齐）
...
3、**如果一个盒子没有给定宽度，则会使用父亲的宽度，不算继承，因为块级元素本身就占一整行，如果不指定宽度，则padding不会影响本盒子大小，但是margin会，但都不会改变父盒子的大小，如果子盒子自定义的宽度超过了父盒子的宽度，则会超出父盒子，不会被限制。**

​	margin的情况

![1532660377344](C:\Users\16428\Desktop\笔记\CSS笔记\media\1532660377344.png)

![1532660366444](C:\Users\16428\Desktop\笔记\CSS笔记\media\1532660366444.png)

padding的情况

![1532660411442](C:\Users\16428\Desktop\笔记\CSS笔记\media\1532660411442.png)

![1532660430633](C:\Users\16428\Desktop\笔记\CSS笔记\media\1532660430633.png)
...
1.水平和垂直阴影不是指水平和垂直的阴影长度，而是相对偏移量

![1532662459995](C:\Users\16428\Desktop\笔记\CSS笔记\media\1532662459995.png)

![1532662472838](C:\Users\16428\Desktop\笔记\CSS笔记\media\1532662472838.png)

2.模糊距离会影响阴影尺寸的显示效果（暂定）
...
（注意是拥有行内块元素的部分特性，而不是变成行内块元素，即使是在float属性的前面或者后面加上display: block;，也不会将这个元素变成块状元素）。
元素的大小完全取决于定义的大小或者默认的内容多少浮动根据元素书写的位置来显示相应的浮动（因为行内块元素的特性）。
```

5.特例

![1532964652870](C:\Users\16428\Desktop\笔记\CSS笔记\media\1532964652870.png)

第一个div浮动了，第二个div没有浮动，第二个div内的文字没有上去（问题暂定）

![1532964701498](C:\Users\16428\Desktop\笔记\CSS笔记\media\1532964701498.png)

暂定结果：文字不能盖在浮动元素下面

![1532965514563](C:\Users\16428\Desktop\笔记\CSS笔记\media\1532965514563.png)

![1532965569577](C:\Users\16428\Desktop\笔记\CSS笔记\media\1532965569577.png)
...
```css
 ...
 .clearfix {*zoom: 1;}   /* IE6、7 专有 */ 除了IE浏览器，其他浏览器看到元素前面有*是不会去执行的
```
...
边偏移量只能4选2，上下、左右，如果定义了top，即使后面有bottom也不会覆盖

![1533225207474](C:\Users\16428\Desktop\笔记\CSS笔记\media\1533225207474.png)
...
静态定位唯一的用处就是用来取消其他定位状态，因为定位模式没有none这个属性值
...
注意：    绝对定位最重要的一点是，它可以通过边偏移移动位置，但是它完全脱标，完全不占位置（跟浮动类似）。

绝对定位是相对于浏览器屏幕（不占有位置，只有绝对定位是完全意义上的脱标，浮动的话脱标后下面的元素如果是图片或者里面包含有文字的话，则图片和元素里面的文字不会移到脱标的位置，但是绝对定位的话可以；绝对定位脱标的位置下面的图片和元素里面的文字可以移上来。）
...
注：如果有爷、父、孙三个div,如果孙div使用了绝对定位，如果父div没有定位，爷div使用了定位，则孙div根据爷div定位
...
普通的盒子是左右margin 改为 auto就可， 但是对于绝对定位和浮动就无效了，magin的auto属性值对浮动和绝对定位的元素没有效果，不管是在浮动还是定位前，但是其他属性值还是有效果的，下面的margin-left就是一个例子。
...
跟 浮动一样， 元素添加了 绝对定位和固定定位之后， 元素模式也会发生转换， 都转换为 行内块模式（如果没有自定义高度和宽度，则行内块的宽度和高度由内容决定，需要特别注意的是，插入图片并不会撑开盒子，也就是插入的图片没办法改变行内块的高度和宽度），
...
### 轮廓 outline
 ...
 是绘制于元素周围的一条线，位于边框边缘的外围，可起到突出元素的作用（比如移到表单边框上时显示的边框效果）。
