...
capacity：当前数组容量，始终保持 2^n，可以扩容，扩容后数组大小为当前的 2 倍。（默认是16）
...
// 这个很简单，其实就是将新值放到链表的表头，原来的表头放到新节点的后面，然后 size++
...
HashMap中的扰动函数（上面的扰动函数是HashMap1.8中的实现）

![](图片/5.png)

hash值是int型，32位的，原hash值的低16位和高16位进行异或运算（混合低16位，让低位也混有高位的特性保证高低位都参与到hash的运算中），最后通过取模。

之所以是要这样是因为原来的hash值太大了。

#### 使用final对象，并重写equals()和hashCode()方法

使用final对象可以保证对象作为key加入到HashMap后hashcode不会发生改变，String就是一个很好的例子，底层数组是fianl，而且还重写了equals()和hashCode()

![](图片/2.png)

![](图片/3.png)

Object默认的hashCode()是本地方法，是通过内存地址计算hash值的，而equals则比较的是地址。
...
public ConcurrentHashMap(int initialCapacity,
    ...
    // 插入一个元素不至于扩容，插入第二个的时候才会扩容(因为阀值是1.5)
...
final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
    ...
    if ((p = tab[i = (n - 1) & hash]) == null)
        ...
        // 注意不是onlyIfAbsent才可以进行值覆盖，旧值为null也可以进行值覆盖
        if (e != null) {
            V oldValue = e.value;
        ...
        for (int j = 0; j < oldCap; ++j) {
            ...
            if ((e = oldTab[j]) != null) {
                ...
                // 如果是红黑树，具体我们就不展开了
                    ...
                    /*
                    由于扩容数组的长度是 2 倍关系，所以对于假设初始 tableSize = 4 要扩容到 8 来说就是 				  0100 到 1000 的变化（左移一位就是 2 倍），在扩容中只用判断原来的 hash 值与左移动的一					位（newtable 的值）按位与操作是 0 或 1 就行，0 的话索引就不变，1 的话索引变成原索引加				  上扩容前数组。
                    */
...
ConcurrentHashMap不允许插入key或者value为null
...
Java8中的ConcurrentHashMap引入了CAS+synchronized使锁更细化
...
sizeCtl（大小控制）是volatile修饰的变量，多线程下保证了可见性，-1代表有一个线程正在初始化或扩容操作，-n表示有（n-1）个线程正在初始化或者扩容，整数或者0则代表哈希表还没被初始化，这个数值表示初始化或者下一次扩容的大小。
