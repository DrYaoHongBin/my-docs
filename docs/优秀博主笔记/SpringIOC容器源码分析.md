...
自己总结的BeanFactory和Application区别：

1. 首先两者加载Bean的方式都是通过Xml配置文件，ApplicationContext和BeanFacotry相比，提供了更多的扩展功能，但是这还不是主要区别，主要区别在于BeanFacotry是延迟加载，举个例子：如果Bean没有完全注入，BeanFacotry加载后，会在你第一次调用GetBean方法才会抛出异常；而ApplicationContext会在初始化的时候就加载并且检查，这样的好处是可以及时检查依赖是否完全注入；所以通常来说我们会选择使用ApplicationContext。 
...
3. BeanFacotry是Spring中相当古老的Factory了，比如说Xml BeanFactory就是一种相当典型的BeanFactory。所以原始的BeanFactory就无法支持诸如AOP、Web应用等许多插件。 
4. ApplicationContext接口是由BeanFactory接口派生出来的，所以提供了BeanFactory的所有功能。ApplicationContext是一种更加面向框架的工作方式以及对上下文进行分层和实现继承。并且ApplicationContext还额外的提供了以下功能： •MessageSource，提供国际化的消息访问；资源访问，如URL和文件等。
...
启动过程参考链接：https://www.jianshu.com/p/93874bb2844d

Aware接口参考链接：https://www.jianshu.com/p/c5c61c31080b

ApplicationListener接口参考链接：https://blog.csdn.net/liyantianmin/article/details/81017960
...
public class ClassPathXmlApplicationContext extends AbstractXmlApplicationContext {
  ...
  ...
  // 如果是普通的new (String),那configLocations只有一个元素，就是配置文件的路径，refresh为true,parent为null    
...
```java
// refresh()是AbstractApplicationContext里面的方法
@Override
public void refresh() throws BeansException, IllegalStateException {
   ...
   */
   synchronized (this.startupShutdownMonitor) {
      ...
      // 准备工作，记录下容器的启动时间、标记容器“已启动”状态、初始化properties资源，处理配置文件中的占位符
      prepareRefresh();
      ...
      // Application内组合了BeanFactory
      // 为内部的BeanFactory设置类加载器，添加几个 BeanPostProcessor，手动注册几个特殊的 bean
      /*
      	BeanPostProcessor(ApplicationContextAwareProcessor,ApplicationListenerDetector),
	  ApplicationContextAwareProcessor:对Spring 中各种Aware接口的支持,在初始化Bean前，调用Bean实现的	  Aware接口方法。
	  ApplicationListenerDetector：用于收集实现了ApplicationListener接口的Bean
      */
      prepareBeanFactory(beanFactory);
       
      /*
      	BeanFactoryPostProcesssor和BeanPostProcessor的区别在于，BeanFactoryPostrProcessor能在Bean初始化之前获取整个beanFactory，对BeanFactory进行加强，例如修改bean的配置信息。
      	BeanPostProcessor则是在Bean构造完成，调用init方法前后执行，每次只能获取一个bean。
      */
...
public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
   ...
   // 用一个 ThreadLocal 来存放配置文件资源，其实是为了防止重复加载文件
   /*
...
首先会回调几个实现了 Aware 接口的 bean，然后就开始回调 BeanPostProcessor 的 postProcessBeforeInitialization 方法，之后是回调 init-method，注意init-method是在对象的构造方法完成之后才调用，然后再回调 BeanPostProcessor 的 postProcessAfterInitialization 方法。
